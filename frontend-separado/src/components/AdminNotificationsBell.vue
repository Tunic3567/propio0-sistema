...
async function eliminarSeleccionadas() {
  ...
  if (!window.confirm(t('admin.notifications.confirmDeleteSelected', { n: seleccionados.value.length }))) return
  await ocultarIds([...seleccionados.value])
}

async function eliminarTodas() {
  if (items.value.length === 0) return
  if (!window.confirm(t('admin.notifications.confirmDeleteAll'))) return
  await ocultarIds(items.value.map((i) => i.id))
}
