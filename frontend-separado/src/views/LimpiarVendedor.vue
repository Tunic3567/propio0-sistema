  ...
  <div class="min-h-screen w-full max-w-full overflow-x-clip bg-amber-50/80 dark:bg-slate-900 transition-theme">
    ...
    <div class="p-4 md:p-6 max-w-6xl mx-auto pt-20 space-y-6">
      <div class="rounded-xl border-2 border-red-500/80 bg-red-100/90 dark:bg-red-950/40 dark:border-red-600 p-4 text-red-950 dark:text-red-100">
        <h1 class="text-lg font-bold">Limpiar vendedor (solo prueba)</h1>
        <p class="text-sm mt-2 leading-relaxed">
          Elimina <strong>todos los datos</strong> de un vendedor: clientes, rutas, pagos, ingresos,
          egresos, ventas, notas, historial, c&eacute;dulas reportadas y solicitudes de llave.
          El vendedor como usuario <strong>no se elimina</strong>, solo sus datos operativos.
        </p>
        <p class="text-sm mt-1 font-bold">Solo para usuarios de prueba. Nunca usar en casos reales.</p>
      </div>
      ...
      <template v-if="cargado">
        <div class="rounded-xl border-2 border-neutral-300 dark:border-slate-600 bg-white dark:bg-slate-800 p-4 space-y-4 shadow-sm">
          ...
          >
            ...
            <option v-for="v in vendedores" :key="v._id" :value="v._id">
              {{ v.nombre }} ({{ v.usuario }}) &mdash; {{ v.pais || '?' }}, {{ v.ciudad || '?' }}
            </option>
          </select>
        </div>
        ...
        <div v-if="paso === 0" class="text-center">
          <button
            ...
            :class="vendedorId ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-neutral-300 dark:bg-slate-700 text-neutral-500 dark:text-slate-400 cursor-not-allowed'"
            @click="paso = 1"
          >
            1. Iniciar proceso de limpieza
          </button>
          <p v-if="!vendedorId" class="text-sm text-neutral-400 dark:text-slate-500 mt-2">
        ...
        <div v-if="paso >= 1" class="rounded-xl border-2 border-red-300 dark:border-red-700 bg-red-50 dark:bg-red-950/30 p-4 space-y-3">
          ...
          <p class="text-sm text-red-700 dark:text-red-300">
            Est&aacute;s a punto de eliminar TODOS los datos operativos de
            <strong>{{ vendedorSeleccionado?.nombre }}</strong>.
        ...
        <div v-if="paso >= 2" class="rounded-xl border-2 border-red-500 dark:border-red-600 bg-white dark:bg-slate-800 p-4 space-y-4 shadow-lg">
          ...
          <div class="border-t border-red-200 dark:border-red-800 pt-4 space-y-3">
            <p class="text-sm font-bold text-red-700 dark:text-red-300">
              Para ejecutar, escribe "LIMPIAR" en el campo de abajo y haz clic en el bot&oacute;n rojo.
            </p>
            <input
            ...
            <div class="flex flex-wrap gap-3">
              <button
                :disabled="textoConfirmacion !== 'LIMPIAR' || ejecutando"
                class="px-8 py-3 rounded-xl text-lg font-bold transition-colors"
                :class="textoConfirmacion === 'LIMPIAR' && !ejecutando ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-neutral-300 dark:bg-slate-700 text-neutral-500 dark:text-slate-400 cursor-not-allowed'"
                @click="ejecutarLimpieza"
              >
                {{ ejecutando ? 'Ejecutando...' : '3. Ejecutar limpieza &mdash; &iexcl;IRREVERSIBLE!' }}
              </button>
              <button
                v-if="!ejecutando"
        ...
        >
          ...
          >
            {{ resultado.ok ? 'Limpieza completada' : 'Error' }}
          </h3>
...
const vendedorSeleccionado = computed(() => vendedores.value.find(v => v._id === vendedorId.value))
...
onMounted(async () => {
  try {
    ...
    if (res.ok) {
      vendedores.value = (data.vendedores || data || []).filter(v => v._id)
    }
  } catch (e) {
...
async function confirmarPaso2() {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/api/admin/super/vendedor/${vendedorId.value}/limpiar`, {
      method: 'POST',
      headers: {
