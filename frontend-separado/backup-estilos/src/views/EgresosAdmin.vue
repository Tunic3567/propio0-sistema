  ...
  <div class="min-h-screen bg-gray-100">
    ...
    <div class="p-4 max-w-5xl mx-auto">
      <div class="flex flex-wrap gap-4 mb-6">
        <div>
          <label class="block font-semibold mb-1">{{ $t('admin.filterByVendor') || 'Filtrar por vendedor' }}:</label>
        ...
        <div>
          <label class="block font-semibold mb-1">{{ $t('admin.filterByRoute') || 'Filtrar por ruta' }}:</label>
      ...
      <div class="bg-white rounded shadow p-6">
        <h2 class="text-lg font-bold mb-4">{{ $t('expense.registered') }}</h2>
        <div v-if="loading" class="text-center">{{ $t('common.loading') }}</div>
        <div v-else-if="egresos.length === 0" class="text-center text-gray-500">{{ $t('expense.noMovements') }}</div>
