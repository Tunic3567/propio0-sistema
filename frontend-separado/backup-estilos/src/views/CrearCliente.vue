      ...
      <Teleport to="body">
        <div v-if="mostrarModalCedulaReportada" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full border-2 border-yellow-300/50 dark:border-yellow-700/50 transition-all duration-300">
            <div class="p-6 border-b-2 border-gray-200 dark:border-gray-700 bg-gradient-to-r from-yellow-50 to-white dark:from-gray-800 dark:to-gray-800 rounded-t-2xl">
...
// Sin mapa: buscamos/obtenemos ubicación y guardamos coordenadas
  ...
  // Validar si la cédula está reportada antes de crear el cliente
  if (cliente.value.cc) {
    try {
      ...
      if (codigoVinculacion) {
        ...
        if (dataVerificar.reportada) {
          ...
          datosClienteParaCrear.value = nuevoCliente; // Guardar datos para crear después
          mostrarModalCedulaReportada.value = true;
          return; // Detener aquí y esperar confirmación del usuario
        }
      }
      // Si no hay código de vinculación, continuar con la creación normalmente
    } catch (e) {
      // Si falla la verificación, continuar con la creación (no bloquear)
      console.error('Error al verificar Id reportado:', e);
    }
  }

  // Si no hay Id reportado, proceder directamente con la creación
  procederConCreacion(nuevoCliente);
}
...
function continuarConCreacion() {
  mostrarModalCedulaReportada.value = false;
  if (datosClienteParaCrear.value) {
    procederConCreacion(datosClienteParaCrear.value);
    datosClienteParaCrear.value = null; // Limpiar después de usar
  }
}
